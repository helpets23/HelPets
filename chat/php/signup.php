                ...
                if (in_array($img_ext, $extensions)) {
                    ...
                    if (move_uploaded_file($tmp_name, "images/" . $new_img_name)) {
                       $status = "Ativo agora";
                        ...
                        $sqlInsert = mysqli_query($conn, "INSERT INTO user (unique_id, fname, lname, email, password, img, status)
                                                          VALUES ({$unique_id}, '{$fname}', '{$lname}', '{$email}', '{$password}',
                                                                  '{$new_img_name}', '{$status}')");
                        if ($sqlInsert) {
                            $sql = mysqli_query($conn, "SELECT unique_id FROM user WHERE email = '{$email}'");
                            if (mysqli_num_rows($sql) > 0) {
